    ...
    function createNewBill($INPUTS=[]){
        ...
        if($res = mysqli_query($dbCon, $sqlBill)){
            ...
            foreach($itemNameList as $itemKey=>$itemName){
                ...
                $sqlBillItem = "INSERT `".ERP_PURCHASE_BILLS_ITEMS."` SET `bill_id`='".$billId."',`item_id`='".$itemId."',`bill_item_desc`='".$itemDescription."',`bill_item_qty`='".$itemQuantity."',`bill_item_price`='".$itemUnitPrice."', `bill_item_total_price`='".$itemTotalPrice."', `bill_item_created_by`='".$adminId."', `bill_item_updated_by`='".$adminId."',`bill_item_status`='active'";
